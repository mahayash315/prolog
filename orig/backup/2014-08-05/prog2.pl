...
penalty('dependent', Tid, Cid, Dchunk, Weight) :-
    ...
    W0 is D - 10,
    (W0 <  0, Weight is W0 * (-1.1), !;
     W0 =  0, Weight is 0, !;
      0 < W0, Weight is W0 * 0.01).
%    (1 =< D,  Weight is abs(D-10), !;
%     D < 1,  Weight is abs(D)+100).
...
select_move_cell(LastRow,LastCol,Row,Col,P) :-
    (retractall(p_of(_,_,_)), !; true),
    (cell(LastRow,LastCol,Ltid,Lcid), !; Ltid = '-', Lcid = '-'), !,
    forall(penalty(Tid,Cid,P), (Tid \== Ltid, Cid \== Lcid, assert(p_of(Tid,Cid,P)), !; true)),
...
solve_loop(RemItr,LastRow,LastCol,FinalEval) :-
    ...
    (
    	...
    	writef('choose cell to move: (%t-%t) [%t,%t] (P=%t)',[Tid,Cid,FromRow,FromCol,FromP]),nl,
    	(
			% ループの終了条件
			%FromP = 0, FinalTotalEv = Eval, !, true;
			%0 < LastTotalEv, LastTotalEv < TotalEv, FinalTotalEv = LastTotalEv, !, true;
			% 移動先の検討
			decide_move_to(FromRow,FromCol,FromP,ToRow,ToCol,ToP),
			%print_cells, nl,
			...
			(
			    ...
			    ToP =< FromP,
			    % 移動
				%writef('[%5r] moving [%t,%t] -> [%t,%t], FromP=%t, ToP=%t',[RemItr,FromRow,FromCol,ToRow,ToCol,FromP,ToP]),nl,
			    swap_cell(FromRow,FromCol,ToRow,ToCol),
...
print_cell(Row,Col) :-
    ...
    (constraint(Tid,_,'deadline',cell(Row,Col,_,_)), !, writef('<%t>', [Tid]);
     %cost_at(Row,Col,Cost), !, writef('%4c',[Cost]);
     writef('%5c',['-'])).
print_cell(Row,Col) :-
