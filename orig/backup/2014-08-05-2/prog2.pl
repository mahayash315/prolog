...
penalty('deadline', Tid, Cid, Cell, Weight) :-
    ...
    W0 is (D+Cid),
    ( 0  < W0, Weight is 0, !;
     W0 =<  0, Weight is abs(W0) * 101).

penalty('dependent_from', Tid, Cid, Dchunk, Weight) :-
    Dchunk = chunk(Dtid,Dcid,_),
    cell(Mrow,Mcol,Tid,Cid),
    cell(Drow,Dcol,Dtid,Dcid),
    cell_distance(Drow,Dcol,Mrow,Mcol,D),
    W0 is (D+Cid) - 10,
    (W0 <  0, Weight is W0 * (-10), !;
     W0 =  0, Weight is 0, !;
      0 < W0, Weight is W0 * 0.1).
penalty('dependent_to', Tid, Cid, Dchunk, Weight) :-
	Dchunk = chunk(Dtid,Dcid,_),
	cell(Mrow,Mcol,Tid,Cid),
	cell(Drow,Dcol,Dtid,Dcid),
	cell_distance(Drow,Dcol,Mrow,Mcol,D),
	W0 is (D-Cid) + 10,
	(W0 <  0, Weight is W0 * (-0.1), !;
	 W0 =  0, Weight is 0, !;
	  0 < W0, Weight is W0 * 10).

penalty('sequential_prev', Tid, Cid, Chunk, Weight) :-
    Chunk = chunk(Ptid,Pcid,_),
    cell(Crow,Ccol,Tid,Cid),
    cell(Prow,Pcol,Ptid,Pcid),
    cell_distance(Prow,Pcol,Crow,Ccol,D),
    W0 is D - 1,
    (W0 <  0, Weight is W0 * (-20), !;
     W0 =  0, Weight is 0, !;
      0 < W0, Weight is W0 * 0.2).
%    (D = 1, Weight is 0, !;
%     1 < D, Weight is 0.1*D, !;
%     D < 1, Weight is 0.1*abs(D)+10).

penalty('sequential_next', Tid, Cid, Chunk, Weight) :-
    Chunk = chunk(Ntid,Ncid,_),
    cell(Crow,Ccol,Tid,Cid),
    cell(Nrow,Ncol,Ntid,Ncid),
    cell_distance(Nrow,Ncol,Crow,Ccol,D),
    W0 is D + 1,
    (W0 <  0, Weight is W0 * (-0.22), !;
     W0 =  0, Weight is 0, !;
      0 < W0, Weight is W0 * 22).
%    (D = 1, Weight is 0, !;
%     1 < D, Weight is 0.1*D, !;
%     D < 1, Weight is 0.1*abs(D)+10).
...
prepare_constraints :-
    ...
    forall(chunk(Tid,1,_),(
		   ...
		   (
		       ...
		       assert(constraint(Tid,Cid,'deadline',cell(Row,Col,-,-)))
		       ;
		       true
		   )
	       )
    ...
    forall(chunk(Tid,Cid,_),(
	       (PrevCid is Cid-1, chunk(Tid,PrevCid,Len), !, assert(constraint(Tid,Cid,'sequential_prev',chunk(Tid,PrevCid,Len))); true),
	       (NextCid is Cid+1, chunk(Tid,NextCid,Len), !, assert(constraint(Tid,Cid,'sequential_next',chunk(Tid,NextCid,Len))); true)
	  )),
    % 固定スケジュールの制約
...
prepare_constraints('dependent',Tid,[Dtid|Rest]) :- !,
	...
	forall(chunk(Tid,Cid,Len), (
			assert(constraint(Tid,Cid,'dependent_from',chunk(Dtid,Dcid,Dlen))),
			assert(constraint(Dtid,Dcid,'dependent_to',chunk( Tid, Cid, Len)))
		))
	; true
    ),
    % 次の依存制約を処理
...
decide_move_to(FromRow,FromCol,FromP,ToRow,ToCol,ChosenCost) :-
    ...
    length(Costs,Len), Max is min(Len,3),
    random(0,1,Index),
    nth(Costs,Index,Val),
...
solve_loop(RemItr,LastRow,LastCol,FinalEval) :-
    0 < RemItr,
    % 今の評価値
    eval(Eval),
    %cls,
    location(1,1),
    writef('Remaining Iteration: %t, Eval = %t',[RemItr,Eval]), nl,
    %print_cells, nl,
    (	get_single_char(_),
   		% 動かすセルの決定
    	...
    	(
			...
			% 移動先の検討
			decide_move_to(FromRow,FromCol,FromP,ToRow,ToCol,ToEval),
			print_cells, nl,
			...
			(
			    ...
			    %ToP =< FromP,
			    % 移動
				writef('[%5r] moving [%t,%t] -> [%t,%t], FromP=%t, ToEval=%t',[RemItr,FromRow,FromCol,ToRow,ToCol,FromP,ToEval]),nl,
			    swap_cell(FromRow,FromCol,ToRow,ToCol),
			    NextRemItr is RemItr - 1,
			    solve_loop(NextRemItr,ToRow,ToCol,FinalEval), !;
			    % 移動しない
			    %Eval < ToEval,
			    NextRemItr is RemItr - 1,
			    solve_loop(NextRemItr,FromRow,FromCol,FinalEval)
			);
