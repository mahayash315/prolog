...
writeSchedules :-
    ...
    (setof(schedule(X1,X2,true), (X1,X2)^schedule(X1,X2,true), Set); Set=[]),
    writeSchedules(Set), nl, nl,
    write('% dynamic schedules:\n'),
    (setof(schedule(X1,X2,false), (X1,X2)^schedule(X1,X2,false), Set); Set=[]),
    writeSchedules(Set).
...
stamp_to_cell(Timestamp,Row,Col) :-
    ...
    stamp_to_date(ScheduleFrom,Y1,M1,D1),
    stamp_to_date(Timestamp,Y2,M2,D2),
    count_days(Y1,M1,D1,Y2,M2,D2,Col),
    get_day_span(DayFrom,DayTo),
    stamp_to_time(Timestamp,H,I,_),
    Time is H*100+I,
    DayFrom =< Time, Time =< DayTo,
    count_rows(DayFrom,Time,Row).

cell_to_stamp(Row,Col,Timestamp) :-
    get_schedule_span(ScheduleFrom,ScheduleTo),
    get_day_span(DayFrom,_),
    stamp_to_date(ScheduleFrom,Y1,M1,D1),
    stamp_to_time(ScheduleFrom,H1,I1,S1),
    Day is D1 + (Col-1),
    date_time_stamp(date(Y1,M1,Day,H1,I1,S1,0,-,-), Timestamp1),
    count_rows(0, DayFrom, RowSize),
    Timestamp is Timestamp1 + RowSize*10*60 + floor(Row / 6)*3600 + ceil(Row mod 6)*10*60,
...
satisfy('sequential', Tid, Cid, Pcid, Weight) :-
...
satisfy('fixed', Tid, Cid, Cell, Weight) :-
    Cell = cell(Row,Col,_,_),
    cell(Row,Col,Tid,Cid), Weight is 0.

satisfy('soft_fixed', Tid, Cid, Cell, Weight) :-
    Cell = cell(Row,Col,_,_),
    (cell(Row,Col,Tid,Cid), !, Weight is 0;
     Weight is 1000).
...
select_move_cell(Row,Col,Ev) :-
    bagof(Ev,(Tid,Cid)^eval(Tid,Cid,Ev),Evs),
    find_max(Evs,Ev),
    eval(Tid,Cid,Ev),
...
decide_move_to(FromRow,FromCol,ToRow,ToCol,Ev) :-
    ...
    forall(eval_when_swapped(FromRow,FromCol,ToRow,ToCol,Ev),
...
eval_when_swapped(FromRow,FromCol,ToRow,ToCol,Ev) :-
    get_cell_size('row',RowSize),
    get_cell_size('col',ColSize),
    between(1,RowSize,ToRow),
    between(1,ColSize,ToCol),
    cell(FromRow,FromCol,Tid,Cid),
    swap_cell(FromRow,FromCol,ToRow,ToCol),
    eval(Tid,Cid,Ev),
    swap_cell(FromRow,FromCol,ToRow,ToCol).

solve_loop(RemItr,LastEv) :-
    0 < RemItr,
    % 動かすセルの決定
    select_move_cell(FromRow,FromCol,FromEv),
    % 移動先の検討
    decide_move_to(FromRow,FromCol,ToRow,ToCol,ToEv),
    write(RemItr),write('  '),write(FromEv),write('-->'),write(ToEv),nl,
    (ToEv < FromEv,
     % 移動
     swap_cell(FromRow,FromCol,ToRow,ToCol),
     NextRemItr is RemItr - 1,
     solve_loop(NextRemItr,LastEv), !;
     % 終了
     LastEv = FromEv,
     true).
