...
ctime(T,H,I) :- between(0,2359,T), T mod 100 < 60, between(0,23,H), between(0,59,I), T is H*100 + I.
ctime_diff(T1,T2,Diff) :- ctime(T1,H1,M1), ctime(T2,H2,M2), Diff is (H2-H1)*6+(M2-M1).
...
cell_stamp(Row,Col,Timestamp) :-
    get_cell_size('row',RowSize),
    get_cell_size('col',ColSize),
    between(1,RowSize,Row),
    between(1,ColSize,Col),
    get_schedule_span(ScheduleFrom,ScheduleTo),
    SFI is integer(ScheduleFrom), STI is integer(ScheduleTo),
    step(SFI,STI,86400,TSI),
    TSF is float(TSI),
    stamp_to_date(ScheduleFrom,Y0,M0,D0),
    stamp_to_time(ScheduleFrom,H0,I0,S0),
    stamp_to_date(TSF,Y1,M1,D1),
    count_days(Y0,M0,D0,Y1,M1,D1,Col),
    get_day_span(DayFrom,DayTo),
    H1 is H0 + floor(((DayFrom/100)+(Row-1))),
    I1 is I0 + ceil((DayFrom mod 100)/10),
    date_time_stamp(date(Y1,M1,D1,H1,I1,S0,0,-,-), Timestamp).
...
prepare_constraints :-
    ...
    forall(schedule(DateTime,Tid,false),(
	       writef('%t %t',[Tid,DateTime]),nl,
	       date_time_stamp(DateTime,Timestamp),
	       (stamp_to_cell(Timestamp,Row,Col), write(Row), !, assert(constraint(Tid,1,'soft_fixed',cell(Row,Col,-,-)));
		writef('!'),nl,true)
	  )).
...
solve_loop(RemItr,LastRow,LastCol,LastEv,FinalEv) :-
    cls, location(1,1), print_cells, nl,
    0 < RemItr,
    % 動かすセルの決定
