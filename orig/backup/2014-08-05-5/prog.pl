...
solve_loop(RemItr,LastRow,LastCol,LastEval,DupCount,Temperature,FinalEval) :-
    ...
    (	%get_single_char(_),
   	% 動かすセルの決定
		...
		%Cost =< FromP,
		% 移動
		nl,writef('[%5r] moving [%t,%t] -> [%t,%t], (FromP=%t, Cost=%t)',[RemItr,FromRow,FromCol,ToRow,ToCol,FromP,Cost]),nl,
		swap_cell(FromRow,FromCol,ToRow,ToCol),
...
print_cells(Flg) :-
    ...
    forall(between(1,RowSize,Row),(
	       forall(between(1,ColSize,Col),(
			  print_cell(Row,Col,Flg), !; true
		     ))
	  )).
print_head(Col) :-
    print_cells_pos(Col*10,0,X,Y),
    location(X,Y),
    writef('%10c',[Col]).
print_left(Row) :-
    print_cells_pos(-2,Row,X,Y),
    location(X,Y),
    writef('%3r',[Row]).
print_cell(Row,Col,Flg) :-
    cell(Row,Col,-,_), !, print_cells_pos(Col*10,Row,X,Y), location(X,Y),
    (constraint(Tid,_,'deadline',cell(Row,Col,_,_)), !, format(atom(Str),'<~w>',[Tid]),writef('%10c', [Str]), !;
     ...
     writef('%10c',['-'])).
print_cell(Row,Col,_) :-
    cell(Row,Col,Tid,Cid), !, print_cells_pos(Col*10,Row,X,Y), location(X,Y),
    format(atom(Str),'[~w-~w]',[Tid,Cid]),writef('%10c',[Str]).
print_cells_pos(Xoffset,Yoffset,X,Y) :- X is 3 + Xoffset, Y is 6 + Yoffset.
