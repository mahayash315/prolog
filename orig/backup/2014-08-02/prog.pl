...
load :-
    ...
    readSchedules,
    seen, !.
retractAllTasks :- (retract(task(_,_,_,_,_)), retractAllTasks, !; true).
retractAllSchedules :- (retract(schedule(_,_,_)), retractAllSchedules, !; true).
...
writeSchedules([S|Rest]) :-
    ...
    writeSchedules(Rest).
writeSchedules :-
    (setof(schedule(X1,X2,X3), schedule(X1,X2,X3), Set); Set=[]),
    writeSchedules(Set).
...
input(S) :-
    ...
    proc_input(S,Input,N),
    (Input = 13, true; input(N)).

proc_input(S,13,S) :- !, proc(S).
proc_input(S,65,N) :- (3 =< S, N is S-1, symbol(N), !; N = S).
proc_input(S,66,N) :- (S =< 3, N is S+1, symbol(N), !; N = S).
proc_input(S, _,S) :- !.

proc(2) :- !, cls, location(1,1), manage_task.
proc(3) :- !, cls, location(1,1), do_schedule.
proc(4) :- !, cls, location(1,1), end.
